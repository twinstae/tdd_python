...
def test_uow_can_retrieve_a_batch_and_allocate_to_it(session_factory):
    ...
    HIPSTER_WORKBENCH = "HIPSTER-WORKBENCH"
    insert_batch(session, "batch1", HIPSTER_WORKBENCH, 100, None)
    session.commit()

    uow = unit_of_work.SqlAlchemyUnitOfWork(session_factory)
    with uow:
        product = uow.products.get(sku=HIPSTER_WORKBENCH)
        print(product.sku)
        print(product.batches)
        print(product.events)
        line = model.OrderLine("o1", HIPSTER_WORKBENCH, 10)
        product.allocate(line)
        uow.commit()

    batchref = get_allocated_batch_ref(session, "o1", HIPSTER_WORKBENCH)
...
def try_to_allocate(order_id, sku, exceptions: List[Exception]):

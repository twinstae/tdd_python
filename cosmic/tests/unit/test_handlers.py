...
class TestAddBatch:
    def test_for_new_product(self):
        ...
        bus.handle(commands.CreateBatch("b1", RANDOM_SKU, 100, None))
        created_product = bus.uow.products.get(RANDOM_SKU)
    ...
    def test_for_existing_product(self):
        ...
        RANDOM_SKU = random_sku()
        bus.handle(commands.CreateBatch("b1", RANDOM_SKU, 100, None))
        bus.handle(commands.CreateBatch("b2", RANDOM_SKU, 99, None))
        assert "b2" in [b.ref for b in bus.uow.products.get(RANDOM_SKU).batches]
...
class TestChangeBatchQuantity:
    def test_changes_available_quantity(self):
        ...
        batch_by_ref = bus.uow.products.get_by_batchref("batch1")
